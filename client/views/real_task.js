...
function pressedSubmit(event, instance, val) {
    ...
    for (i = 1; i <= setSize; i++) {
        ...
        Answers.insert({ "workerID": worker_Id,"assignmentId": assignment_Id, "taskID": task_ID, "forecast": fcast, "points": point, "treatment": treatment, "timestamp": Date.now()});
    }
...
Template.real_task.created = function() {
    this.setNum = new ReactiveVar(1);
    timeSeries.push(Date.now().toString()+' RealTask');
    console.log(timeSeries);
};

Template.real_task.helpers({
    tasks: function() {
...
Template.real_task.rendered = function() {
    $('html,body').scrollTop(0);
    startTime = new Date();
};

Template.real_task.events = {
